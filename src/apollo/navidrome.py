...
from apollo import estools, ratings, settings
...
def _auth_params():
    ...
    return {"u": username, "t": token, "s": salt, "v": "1.16.1", "c": "apollo", "f": "json"}
...
def _subsonic_get(endpoint, params=None, timeout=15):
    ...
    state = operation()
    if "navidrome_http" not in state.cache:
        client = requests.Session()
        state.stack.callback(client.close)
        state.cache["navidrome_http"] = client
    try:
        response = state.cache["navidrome_http"].get(url, params=call_params, timeout=timeout)
        response.raise_for_status()
        payload = response.json()
    except requests.RequestException as exc:
        # Requests exceptions can embed authentication query parameters.
        raise RuntimeError(f"Navidrome request failed ({type(exc).__name__}).") from None
...
def find_song_by_filename(filename):
    ...
    cleaned_stem = re.sub("^\\d+(\\s*-\\s*\\d+)?\\s*-\\s*", "", stem).strip()
    queries = [stem]
...
def _write_fail_log(failed_items):
    """Keep the detailed failure report in private application state."""
    if not failed_items:
        return None
    from apollo.files import atomic_write

    path = settings.current().state_dir / "navidrome-sync-failures.jsonl"
    atomic_write(
        path, "".join(json.dumps(item, ensure_ascii=False) + "\n" for item in failed_items)
    )
    return str(path)
...
def update_all_ratings(verbose=False):
    ...
    if not dbh or not sth:
        return {"total": 0, "updated": 0, "missing_file": 0, "unchanged": 0, "failed": 0}
    sth.execute("SELECT artist, title, calculated_rating FROM apollo_calculated_rating")
    rows = sth.fetchall() or []
    total = len(rows)
    if total == 0:
        logger.info("No rows found in apollo_calculated_rating.")
        return {"total": 0, "updated": 0, "missing_file": 0, "unchanged": 0, "failed": 0}
    ...
    processed = 0
    failed_items = []
    logger.info(f"Starting Navidrome rating sync for {total} songs...")
    for row in rows:
        processed += 1
        ...
        try:
            ...
            if current_rating == nd_rating:
                unchanged += 1
                if verbose:
                    logger.info(f"Unchanged rating {nd_rating}: {artist} - {title}")
            else:
                set_rating_by_song_id(song_id, nd_rating)
                updated += 1
                if verbose:
                    logger.info(f"Updated rating {nd_rating}: {artist} - {title}")
            ...
            if not filename:
                ...
                if verbose:
                    logger.info(f"Missing filename in ES for: {artist} - {title}")
            else:
                try:
                    song = find_song_by_filename(filename)
                    song_id = song.get("id")
                    ...
                    if current_rating == nd_rating:
                        ...
                        if verbose:
                            logger.info(
                                f"Unchanged rating {nd_rating}: {artist} - {title} ({filename})"
                            )
                    else:
                        set_rating_by_song_id(song_id, nd_rating)
                        updated += 1
                        if verbose:
                            logger.info(
                                f"Updated rating {nd_rating}: {artist} - {title} ({filename})"
                            )
                    ...
                    if verbose:
                        logger.info(f"Failed to update {artist} - {title}: {fallback_exc}")
                    elif processed <= 3:
                        logger.info(f"Sample failure {artist} - {title}: {fallback_exc}")
        if not verbose and (processed % 100 == 0 or processed == total):
            logger.info(
                f"Progress {processed}/{total} | updated={updated} unchanged={unchanged} missing_file={missing_file} failed={failed}"
            )
    ...
    if failed_items:
        logger.info(f"Failure log written: {fail_log_path}")
